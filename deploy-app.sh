...
write_backend_env() {
  ...
  [[ -d "${APP_DIR}/backend" ]] \
    || die "backend/ directory not found at ${APP_DIR}/backend — is APP_DIR correct?"

  # Write to a temp file first so a partial write never leaves a broken .env
...
EOF
  ...
  [[ -s "$tmp_env" ]] \
    || die "backend/.env write failed — file is empty. Check disk space and permissions on ${APP_DIR}/backend/"
...
create_kafka_topics() {
  ...
  sudo -u "$REAL_USER" bash -c "
    ...
    export NODE_ENV=development KAFKA_SSL_ENABLED=false
    '${NODE_BIN}' '${APP_DIR}/backend/scripts/create-topics.js'
...
start_pm2() {
  ...
  # PM2 starts Node with --cwd set to backend/, so dotenv.config() finds .env there.
  # We also pass the NODE_ENV via --env so PM2 preserves it across restarts.
  sudo -u "$REAL_USER" env $pm2_env \
  ...
  # Wait for backend to accept connections before Keycloak provisioning starts
  info "Waiting for backend to be ready on :${BACKEND_PORT}..."
  local i=0
...
print_summary() {
  ...
  if [[ -n "$SUPER_ADMIN_EMAIL" ]]; then
    echo -e "    1. Log in at ${CYAN}${BASE_URL}${NC}"
    echo -e "       Email: ${SUPER_ADMIN_EMAIL}"
    echo -e "       Password: (as set in SUPER_ADMIN_PASSWORD)"
  else
    echo -e "    1. Open ${CYAN}${kc_admin_url}${NC} to create your first user"
    echo -e "    2. Log in at ${CYAN}${BASE_URL}${NC}"
  fi
  if [[ "$DEPLOY_MODE" == "domain-ssl" ]]; then
    echo -e "    • Certbot auto-renewal: verify with  certbot renew --dry-run"
  fi
  echo ""
}
...
main() {
  ...
  derive_urls
  generate_secrets
  write_backend_env      # backend/.env  — must exist before any node script runs
  write_frontend_env     # root .env     — read by Vite at build time
  setup_dirs             # uploads/, tmp/, data/, logs/

  # ── Phase 3: Install backend deps + run migrations ───────────────────────────
  install_backend_deps   # npm ci in backend/  (needed by migrate.js)
  run_migrations         # create DB schema (idempotent)

  # ── Phase 4: Start the backend ───────────────────────────────────────────────
  deploy_face_service    # copy face_quality_service.py, restart face-service
  start_pm2              # PM2: start frs-backend, install systemd unit

  # ── Phase 5: Verify backend is up, then provision Keycloak ───────────────────
  provision_keycloak     # realm, clients, roles, super-admin (idempotent)
  create_kafka_topics    # 10 topics (idempotent)

  # ── Phase 6: Build frontend + configure nginx ─────────────────────────────────
  build_frontend         # npm run build → dist/
  configure_nginx        # generate nginx config, enable site, reload
  setup_ssl              # certbot or self-signed (SSL modes only)
